...
import { CHEVRON_LEFT, CHEVRON_RIGHT } from '../assets/ReactIcons'
import clsx from 'clsx'
import type { ProjectsType } from '../types/types'

export default function useCarousalClicks(projects: ProjectsType) {
  ...
  const handleNextClick = (incrementBy: number) => {
    setActiveIndex(
      activeIndex === projects.length - incrementBy
        ? 0
        : activeIndex + incrementBy
    )
  }
  ...
  const IndexBubbles = () => (
    <div className="flex w-full justify-center gap-2">
      {projects.map((_: unknown, index: number) => (
        <div
          key={index}
          className={clsx(
